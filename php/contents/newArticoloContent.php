<div id="content-aggiungi-articolo">

<h1>Aggiungi un nuovo Articolo</h1>

<p>Per permettere una corretta stesura dell'articolo in modo che rispetti le regole dell'accessibilità
    ricordati di inserire le parole straniere nel modo seguente: [lingua=parola].<br/>
    Esempio: "In conclusione si può affermare che durante le fasi di [En=gameplay] il gioco risulta
    molto gratificante."
</p>

<form action="" method="post" id="" >
    <fieldset class="form-new-articolo">
        <legend class=""> 
            <h2>Campi dati obbligatori per l'aggiunta o modifica di un articolo</h2> 
        </legend>
    <ul class="form-container-datiarticolo">
        <li>
            <label for="aggiungi-titolo">Titolo Articolo</label>
            <input type="text">
        </li>
        <li>
            <label for="aggiungi-immagine">Aggiungi una immagine riguardante il gioco</label>
            <input type="file">
        </li>
        <li>
            <label for="aggiungi-voto">Voto da assegnare al Videogame</label>
            <input type="text" style="width:2em;">
        </li>
        <li>
            <label for="aggiungi-tempo">Tempo necessario per finire il gioco</label>
            <input type="text" style="width:2em;">
        </li>
        <li>
            <label for="aggiungi-recensione">Inserisci il Testo Recensione</label>
            <textarea name="" id="" cols rows style="width:100%; height:15em;"></textarea>
        </li>
        <li>
            <input type="submit" value="Aggiungi">
        </li>

    </ul>

    </fieldset>

</form>


</div>